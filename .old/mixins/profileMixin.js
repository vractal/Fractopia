  ...
  methods: {
    getProfile: async function(webId) {
      ...
      try {
        ...
        profile.organization = p.getString(
    ...
    getWorkspaces: async function(webId) {
      let workspaces = [];
      let storage = await this.solid.data[webId].storage;
      ...
      if (fc.itemExists(indexFile)) {
        ...
        try {
          ...
          let subjects = workspacesDoc.getAllSubjectsOfType(
    ...
    getIndexes: async function(webId) {
      let indexes = {
        puti: { instances: [], classes: [] },
        prti: { instances: [], classes: [] },
      };
      try {
        let profileDoc = await fetchDocument(webId);
        const subject = profileDoc.getSubject(webId);

        let puti = await subject.getNodeRef(
          "http://www.w3.org/ns/solid/terms#publicTypeIndex"
        );
        let prti = await subject.getNodeRef(
        ...
        //    console.log(putiDoc)
        //    console.log(prtiDoc)
        let puIndexes = await putiDoc.findSubjects(
        ...
        puIndexes.forEach(async function(index) {
          let classe = await index.getRef(
            "http://www.w3.org/ns/solid/terms#forClass"
          );
          let instance = await index.getRef(
          ...
          indexes.puti.instances.push({
            instance: instance,
            classe: classe,
            label: label,
            created: created,
          });
          indexes.puti.classes[classe] == undefined
            ? (indexes.puti.classes[classe] = [])
            : "";
    ...
    makeToast(title, content, variant = null) {
      ...
      //   solid: true
      // })
      console.log("TOAAST", content, variant);
    },
  },
};
